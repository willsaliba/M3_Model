...
def load_ldr_data(ldr_dir: Path, num_augments_per_file, is_eval_set):
    ...
    for src_file in src_files:
        #progress update
        if len(all_lines) != 0 and len(all_lines) % 1000 == 0: 
...
def process_file(file_lines, all_lines, num_augments_per_file, label_token, bricks_per_window=40):
    """
    Cleans up LDR by:
...
def load_model_with_timer(config, vocab_size, n_positions=1536):
    """
...
def theMain(
    ...
    save_model_path: Path = Path("trained_model_new"),
    #tokenizer params
    ...
    m3_tokenizer.pad_token = m3_tokenizer.eos_token

    encoded_output = m3_tokenizer.encode(train_lines[3])
    ...
    model = load_model_with_timer(model_config, m3_tokenizer.vocab_size)
